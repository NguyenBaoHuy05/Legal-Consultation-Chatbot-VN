...
async def update_gemini_key(
    ...
    await db.users.update_one(
        ...
        {"$set": {"gemini_api_key": encrypted_key}},
    )
...
async def upgrade_subscription(
    ...
    return {
        "status": "success",
        "message": "Yêu cầu nâng cấp đã được gửi. Vui lòng chờ Admin duyệt.",
    }
...
async def update_user_subscription(
    username: str,
    subscription_type: str = Body(..., embed=True),
    ...
    return {
        "status": "success",
        "message": f"User {username} subscription set to {subscription_type}",
    }
...
async def chat(
    ...
    if not final_gemini_key:
        ...
        if current_user.subscription_type == "premium":
            ...
            if not final_gemini_key:
                raise HTTPException(
                    status_code=503, detail="System Gemini Key not configured."
                )
        else:
            ...
            if last_usage and last_usage >= today:
                if current_user.daily_usage_count >= 5:
                    raise HTTPException(
                        status_code=402,
                        detail="Daily limit reached. Please upgrade or add your own API Key.",
                    )
                ...
                await db.users.update_one(
                    {"username": current_user.username},
                    {
                        ...
                        "$set": {"last_usage_date": datetime.utcnow()},
                    },
                )
            else:
                # Reset and increment
    ...
    if not final_gemini_key:
        raise HTTPException(
            status_code=400,
            detail="Please configure your Gemini API Key in settings or upgrade to Premium.",
        )

    if not rag_system:
        raise HTTPException(
            status_code=503,
            detail="System RAG (Pinecone) not configured by Admin.",
        )
        ...
        formatted_sources.append(
            {
                ...
                + 1,  # Convert 0-index to 1-index for display
            }
        )
    ...
    existing_conv = await conversations_collection.find_one(
        {"session_id": request.session_id, "user_id": current_user.username}
    )
    ...
    return {"response": response_text, "sources": formatted_sources}
...
async def download_template_endpoint(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error downloading template: {str(e)}"
        )
...
async def chat_contract(
    ...
    response_raw = bot.generate_response_contract(
        request.message,
        request.variables,
        request.messages,
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Gemini JSON parse error: {str(e)}"
        )
    if response_json.get("status") == "complete":
        # Generate contract document
        try:
            output_path = fill_contract(
                "output_template.docx", response_json.get("variables", {})
            )
            return {
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error generating contract document: {str(e)}",
            )
    return {
...
async def upload_files(
    ...
    try:
        ...
        if documents:
            ...
            raise HTTPException(
                status_code=400, detail="No documents processed"
            )
    finally:
...
async def save_template(
    file: ContractFile, current_user: User = Depends(get_current_admin_user)
):
    # save file to db
...
async def upload_files_supabase(
...
async def admin_config(
    config: ConfigRequest, current_user: User = Depends(get_current_admin_user)
):
...
async def update_user_status(
    ...
    if username == current_user.username:
        raise HTTPException(
            status_code=400, detail="Cannot change your own status"
        )
    ...
    return {
        "status": "success",
        "message": f"User {username} disabled status set to {disabled}",
    }
...
async def delete_all_data(current_user: User = Depends(get_current_admin_user)):
    ...
    return {
        "status": "success",
        "message": "All chat history and Pinecone data deleted",
    }
...
async def download_file(
    filename: str, current_user: UserInDB = Depends(get_current_active_user)
):
