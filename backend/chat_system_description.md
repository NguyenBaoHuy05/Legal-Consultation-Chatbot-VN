...
## Tổng quan
Hệ thống chat được thiết kế để cung cấp cho người dùng trải nghiệm hội thoại liền mạch. Nó tích hợp nhiều thành phần, bao gồm frontend, backend, RAG (Retrieval-Augmented Generation) engine, cơ sở dữ liệu vector Pinecone và Google Generative AI (Gemini).

## Luồng hoạt động
1. **Tương tác người dùng**:
   - Người dùng gửi truy vấn chat thông qua giao diện frontend.
...
4. **RAG Engine**:
   - RAG Engine truy vấn cơ sở dữ liệu vector Pinecone để lấy các vector liên quan.
...
5. **Google Generative AI (Gemini)**:
   - Gemini xử lý truy vấn và tạo phản hồi.
...
- **Google Generative AI (Gemini)**: Tạo phản hồi dựa trên truy vấn và ngữ cảnh.
